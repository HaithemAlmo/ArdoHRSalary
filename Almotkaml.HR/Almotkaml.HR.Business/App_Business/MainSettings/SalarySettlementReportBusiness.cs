...
namespace Almotkaml.HR.Business.App_Business.MainSettings
{
    public class SalarySettlementReportBusiness : Business, ISalarySettlementReportBusiness
    {
        ...
        public bool SearchByDate(SalarySettlementReportModel model)
        {
            ...
            var advancePremiumList = new List<TemEmployeePremiumListItemEE>();
            ...
            else if (model.ISadvanse == 2)
            {
                foreach (var employee in employees)
                {
                    ...
                    foreach (var row in Employeepremiums?.Where(p => p.PremiumId == model.PremiumNameID && p.Value > 0))
                    {
                        ...
                        grid.Add(new EmployeeReportGridRow()
                        {
                            PremiumumName = row.Premium?.Name,
                            EmployeeName = row.Employee.GetFullName(),
                            JobNumber = employee.JobInfo?.JobNumber.ToString(), ///////
                            Value = row.Value,
                            InstallmentValue = row.Value,
                            //Date = row2.DeductionDate,

                            Rest = 0,// row2.AllValue - row2.Value + advancePayments.Salaries.Sum(s => s.AdvancePremiumInside)
                        ...
                        if (salary.IsSuspended == false)
                        {
                            ...
                            if (model.SalarySettlement == SalarySettlement.Summary)
                            {
                                ...
                                foreach (var premiumChecks in PremiumCheckList)
                                {
                                    ...
                                    if (advancePayment.Any(a => a.PremiumId == premiumChecks.PremiumId))
                                        advancePremiumList.Add(new TemEmployeePremiumListItemEE()
                                        {
                                            EmployeeId = employee.EmployeeId,
                                            PremiumId = premiumChecks.PremiumId,
                                            Value = PremiumValue2 ?? 0,
            ...
            //}
            //else 
             if (model.SalarySettlement == SalarySettlement.SalaryForm)
                model.PremiumCheckListItem = PremiumCheckListValues;

            model.PremiumListReport = PremiumCheckListReport2;       
            model.EmployeePremiumList = employeePremiumList;
            model.AdvancePaymentList = advancePremiumList;
        ...
        public bool SearchByDateByID(SalarySettlementReportModel model)
        {
            //if (model.SalarySettlement == SalarySettlement.SalaryForm && model.PremiumCheck)
            //{
            //    if (model.PremiumCheckListItem.Where(e => e.IsSelected == true).Count() > 9)
            ...
            var GetSalary = UnitOfWork.Salaries.GetSalaryByMonth(model.Year ?? 0, model.Month ?? 0).Where(e=>e.EmployeeId==model.EmployeeId);

            var employees = UnitOfWork.Salaries.GetEmployeeReportDate(employeeReportDto).ToList().Where(e=>e.EmployeeId==model.EmployeeId);
            //var banks = UnitOfWork.BankBranches.GetAll().ToList();
            var grid = new List<EmployeeReportGridRow>();
            var holiday = UnitOfWork.Holidays.GetAll().ToList();
            //***************************************************************
            ...
            foreach (var salary in GetSalary)
            {
                ...
                foreach (var Advance in salary?.Employee?.AdvancePayments)
                {
                    datasources.Add(new EmployeeReportGridRow()
                    {
                        DiscountName = Advance.Premium?.Name,
