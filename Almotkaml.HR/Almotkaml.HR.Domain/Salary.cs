...
namespace Almotkaml.HR.Domain
{
    public class Salary
    {
        ...
        //    StampTaxvalue = (TotalSalary(settings) * settings.JihadTax);
        ...
        public decimal NetSalary(ISettings settings)
        {
            ...
            //hh = +(SalaryPremiums.Where(p => p.Premium.IsSubject == false && p.Premium.DiscountOrBoun == DiscountOrBoun.Boun).Sum(p => p.Value));

            //gg = -(SalaryPremiums.Where(p => p.Premium.IsSubject == false && p.Premium.DiscountOrBoun == DiscountOrBoun.Discount).Sum(p => p.Value));

            //dd = TotalSalary(settings) - SolidarityFund(settings)
            //     - JihadTax(settings) - IncomeTax(settings) /*- EmployeeShare(settings)*/ - Grouplife(settings) - Absence() + hh;

            var _vacatio = _VacationDays();
            var sickV = SickVacation(settings);
            var Advance = Employee?.AdvancePayments.Where(s => s.EmployeeId == EmployeeId).Sum(pp => pp.InstallmentValue);
            var fsfsf = (BasicSalary) * ((decimal)_vacatio / 30) * (settings.SickVacation);
            //if (SalaryPremiums.Any(p => p.Premium.IsSubject == IsSubject.IsSubject && (p.Premium.DiscountOrBoun == DiscountOrBoun.Boun || p.Premium.DiscountOrBoun == DiscountOrBoun.Discount)))
            //{
            //    hh = +Math.Round((SalaryPremiums.Where(p => p.Premium.IsSubject == IsSubject.IsNotSubject && p.Premium.DiscountOrBoun == DiscountOrBoun.Boun).Sum(p => p.Value)), 3);

            //    gg = -(SalaryPremiums.Where(p => p.Premium.IsSubject == IsSubject.IsNotSubject && p.Premium.DiscountOrBoun == DiscountOrBoun.Discount).Sum(p => p.Value));

            //    dd = Math.Round(TotalSalary(settings), 3) - Math.Round(SolidarityFund(settings), 3) - Math.Round(JihadTax(settings), 3) - Math.Round(IncomeTax(settings), 3) - Math.Round(EmployeeShare(settings), 3) - Math.Round(Grouplife(settings), 3) + hh + Math.Round(RewindValue);
            //}
            //  dd = Math.Round(TotalSalary(settings), 3) - Math.Round(SolidarityFund(settings), 3) - Math.Round(JihadTax(settings), 3) - Math.Round(IncomeTax(settings), 3) - Math.Round(EmployeeShare(settings), 3) - Math.Round(Grouplife(settings), 3)/* + AllBouns(settings)*/ + Math.Round(RewindValue);
            var Advance1 = Employee?.AdvancePayments.Where(s => s.EmployeeId == EmployeeId).Sum(pp => pp.InstallmentValue);

            dd = TotalSalary(settings) - TotalDiscount(settings)
                    + Math.Round((SalaryPremiums.Where(p => p.Premium.IsSubject == IsSubject.IsSubject && p.Premium.DiscountOrBoun == DiscountOrBoun.Discount).Sum(p => p.Value)), 3)
                ...
                + Math.Round((Employee.Premiums.Where(p => p.Premium.IsSubject == IsSubject.IsNotSubject && p.Premium.DiscountOrBoun == DiscountOrBoun.Boun).Sum(p => p.Value)), 3)
                 + Math.Round((Employee.Premiums.Where(p => p.Premium.IsSubject == IsSubject.IsSubject && p.Premium.DiscountOrBoun == DiscountOrBoun.Discount).Sum(p => p.Value)), 3)
                 + Advance??0
                 /*+CompanyShare(settings)*/;
            //else
            //{
            //    dd = TotalSalary(settings) - SolidarityFund(settings)
            //                      - JihadTax(settings) - IncomeTax(settings); /*- EmployeeShare(settings) /*- Grouplife(settings);*/
            //}
            var value = string.Format("{0:0.000}", Math.Truncate(dd * 1000) / 1000);
        ...
        public decimal TotalSalary(ISettings settings)
        {
            ...
            //else
            basicSalary = (BasicSalary + ExtraValue + ExtraGeneralValue);
